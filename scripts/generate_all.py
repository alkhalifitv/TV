...
def generate_plugins():
    """Load private plugins.json, rewrite fields, and save public plugins.json"""
...
def generate_repo():
    """Generate repo.json metadata"""
    repo = {
        "name": "alkhalifitv.github.io indonesia",
        "description": "Cloudstream Indonesia Repo",
        "iconUrl": REPO_ICON,
        "manifestVersion": 1,
        "pluginLists": [f"{PUBLIC_BASE}/cs3/plugins.json"]
    }
    REPO_OUT.write_text(
        json.dumps(repo, indent=2, ensure_ascii=False),
        encoding="utf-8"
    )
    print("✅ repo.json generated")


def generate_site(public_plugins):
    """Generate index.html with plugin cards"""
    cards = []
...
if __name__ == "__main__":
    public_plugins = generate_plugins()
    generate_repo()
    generate_site(public_plugins)
    print("🎉 All tasks completed (plugins.json, repo.json, index.html)")
